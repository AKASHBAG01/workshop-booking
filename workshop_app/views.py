...
def activate_user(request, key=None):
    ...
    user = request.user
    profile = None  # ✅ SAFE default

    # ✅ Ensure profile exists if user logged in
    if user.is_authenticated:
        profile, created = Profile.objects.get_or_create(user=user)
    ...
    if key is None:
        if user.is_authenticated and profile and not profile.is_email_verified and \
                timezone.now() > profile.key_expiry_time:
            status = "1"
            Profile.objects.get(user_id=profile.user_id).delete()
            User.objects.get(id=profile.user_id).delete()
            return render(request, 'workshop_app/activation.html',
                          {'status': status})

        elif user.is_authenticated and profile and not profile.is_email_verified:
            return render(request, 'workshop_app/activation.html')

        elif user.is_authenticated and profile and profile.is_email_verified:
            status = "2"
            return render(request, 'workshop_app/activation.html',
                          {'status': status})
...
def user_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # ✅ FIX: Create profile BEFORE using it
            from .models import Profile
            profile, created = Profile.objects.get_or_create(user=new_user)

            login(request, new_user)

            user_position = profile.position  # ✅ use profile safely
